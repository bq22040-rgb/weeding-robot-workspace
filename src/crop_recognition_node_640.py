...
class CropRecognitionNode:
    def __init__(self):
        ...
        self.csv_file_path = os.path.join(package_path, 'outputs', 'stalk_positions.csv')
        if not os.path.exists(self.csv_file_path):
            with open(self.csv_file_path, mode='w', newline='') as file:
                writer = csv.writer(file)
                writer.writerow(["X", "Y", "Z"])  # ヘッダーを追加

        # Define transformations
        self.transform = T.Compose([
            # T.ToPILImage(),  # 追加: OpenCV画像をPIL形式に変換
            T.ToTensor(),
        ...
        if self.run_detection:  # Only process depth image when command_task is received
            try:
                ...
                if self.stalk_position is not None:# xとyが計算されているときのみ
                    ...
                    self.stalk_position_pub.publish(stalk_position)
                    self.save_stalk_position_to_csv(x, y, max_depth)#csv用
    ...
    def save_stalk_position_to_csv(self, x, y, z):#csv用
        """Save the stalk position to a CSV file"""
        with open(self.csv_file_path, mode='a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([x, y, z])
        rospy.loginfo(f"Saved stalk position to CSV: ({x}, {y}, {z})")
