        ...
        if self.run_detection:  # Only process depth image when command_task is received
            try:
                ...
                if self.stalk_position is not None:# xとyが計算されているときのみ
                    ...
                    # Define a 100x100 region around the stalk position
                    x_min = max(0, x - 75)
                    ...
                    rospy.loginfo(f"Max depth in 150x150 region around stalk: {max_depth} meters")

                    # Publish the stalk position (x, y, z) including the depth (z)
                    stalk_position = Point()
                    stalk_position.x = (x-160)/1.5 #640×480用に補正
                    stalk_position.y = y/1.5 #640×480用に補正
    ...
    def task_state_callback(self, task_state_msg):
        ...
        if not self.first_stalk_sent:
            ...
            if task_state_msg.data == self.stalk_count:
                rospy.loginfo(f"All stalks sent ({self.stalk_count} total)")
                # 認識を再度開始する
                self.run_detection = True
