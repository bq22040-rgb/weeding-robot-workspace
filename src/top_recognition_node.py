...
try:
    ...
    RS_AVAILABLE = False
    print("[TopRecognition] pyrealsense2 not found. Running without RealSense intrinsics.")
...
class TopRecognitionNode:
    def __init__(self):
        rospy.init_node('top_recognition', anonymous=True)
        ...
        if RS_AVAILABLE:
            try:
                ...
                rospy.loginfo(
                    "TopRecognition: intrinsics set from /camera1/aligned_depth_to_color/camera_info"
                )

            except Exception as e:
                self.depth_intrin = None
                rospy.logwarn(f"TopRecognition: failed to get CameraInfo: {e}")
        else:
            rospy.logwarn("TopRecognition: pyrealsense2 not available. Skip intrinsics setup.")
        ...
        self.seg_pub = rospy.Publisher(
            '/camera1/segmented_image',
            Image,
            queue_size=10
        )

        rospy.loginfo("top.recognition node started")
        rospy.loginfo(
            f"TopRecognition ACT mask enable: {self.act_mask_enable}, "
    ...
    def load_model(self):
        ...
        if os.path.isfile(model_path):
            ...
            model.eval()
            rospy.loginfo("TopRecognition: Model loaded successfully")
        else:
            rospy.logerr(f"TopRecognition: Model checkpoint not found at {model_path}")
    ...
    def depth_callback(self, ros_depth):
        try:
            ...
            rospy.logerr(f"TopRecognition: depth CvBridge error: {e}")
    ...
    def command_callback(self, msg):
        ...
        if start_flag == 1.0:
            self.run_once = True
            rospy.loginfo(f"TopRecognition: start for class {self.target_class}")
    ...
    def image_callback(self, ros_image):
        ...
        try:
            ...
            rospy.logerr(f"TopRecognition: CvBridge error: {e}")
            return
        ...
        try:
            ...
            if world_positions:
                ...
                rospy.loginfo(f"TopRecognition: Published {len(world_positions)} weeds.")

            elif pixel_positions:
                px, py = pixel_positions[0]
                result.data = [float(px), float(py)]
                self.result_pub.publish(result)
                rospy.logwarn("TopRecognition: depth not available, published pixel coords only")

            else:
                rospy.loginfo("TopRecognition: no object detected")

        except Exception as e:
            rospy.logerr(f"TopRecognition: error in image_callback: {e}")
    ...
    def detect_world_coordinates(self, cv_image, pred):
        ...
        if (not RS_AVAILABLE) or (self.depth_intrin is None) or (self.depth_image is None):
            rospy.logwarn("TopRecognition: RS or intrinsics/depth not available, skip 3D deprojection.")
            ...
            for contour in contours:
                ...
                rospy.loginfo(f"TopRecognition: pixel ({cX}, {cY})")
        ...
        for contour in contours:
            ...
            if valid.size == 0:
                rospy.logwarn(
                    f"TopRecognition: no valid depth around ({cX},{cY}), skip 3D for this contour"
                )
                continue
            ...
            if Z_m <= 0:
                rospy.logwarn(f"TopRecognition: invalid Z ({Z_m}) at ({cX},{cY}), skip.")
                continue
            ...
            try:
                ...
                rospy.logerr(f"TopRecognition: rs2_deproject_pixel_to_point error: {e}")
                continue
            ...
            rospy.loginfo(
                f"TopRecognition: pixel ({cX},{cY}) -> camera "
