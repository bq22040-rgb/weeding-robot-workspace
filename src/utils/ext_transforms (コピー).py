...
from torchvision.transforms import GammaTransform
...
class ExtGamma(object):
    ...
    def __call__(self, img, lbl):
        ...
        Returns:
            PIL Image: Gamma image.
        """
        gamma = random.uniform(self.gamma_value[0], self.gamma_value[1])
        return GammaTransform(img)

    def __repr__(self):
        format_string = self.__class__.__name__ + '(gamma={0})'.format(self.gamma)
        return format_string
###
