...
class ExtRandomScale(object):
    def __init__(self, scale_range, interpolation=Image.BILINEAR):
        self.scale_range = (max(scale_range[0], 0.1), scale_range[1])#20241009
    ...
    def __call__(self, img, lbl):
        ...
        # サイズが0以下の場合、スキップするかデフォルト値を使用20241009
        if target_size[0] <= 0 or target_size[1] <= 0:
            print(f"Invalid target size: {target_size}, skipping resize.")
            target_size = (475, 475)  # デフォルトサイズに戻す
        
        #print(f"Resizing Image from {img.size} to {target_size}")  # デバッグ用出力20241009
...
class ExtToTensor(object):
    ...
    def __call__(self, pic, lbl):
        ...
        """
        #####ExGチャンネル
        # b = np.array(pic.getchannel("B"))
        # g = np.array(pic.getchannel("G"))
        # r = np.array(pic.getchannel("R"))
        # exg = 2*g - r - b
        # exg = Image.fromarray(exg)
        # if self.normalize:
        #     pic = torch.cat((F.to_tensor(pic), F.to_tensor(exg)), dim=0)
        #     return pic, torch.from_numpy(np.array(lbl, dtype=self.target_type))
        # else:
        #     pic = torch.from_numpy(
        #         np.array(pic, dtype=np.float32).transpose(2, 0, 1))
        #     exg = torch.from_numpy(
        #         np.array(exg, dtype=np.float32).transpose(2, 0, 1))
        #     return torch.cat((pic, exg.unsqueeze(0)), dim=0), torch.from_numpy(np.array(lbl, dtype=self.target_type))
        ########nomal
        if self.normalize:
