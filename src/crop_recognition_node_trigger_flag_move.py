...
class CropRecognitionNode:
    def __init__(self):
        ...
        # 茎publish関連
        self.first_stalk_sent = False
        self.stalk_count = 0 # 送った茎の数
        self.target_stalk_count = 0 # 検出した茎の数
        ...
        self.task_State_sub = rospy.Subscriber('/task_state_pub', Float64, self.task_state_callback)
        ...
        # 走行部関連
        self.move_flag_pub = rospy.Publisher('/move_flag', Float64, queue_size=10)
        self.move_flag = Float64()
        self.move_flag.data = 0
        self.move_flag_pub.publish(self.move_flag)
        ...
        if self.run_detection:  # Only process depth image when command_task is received
            try:
                ...
                if self.stalk_position is not None:# xとyが計算されているときのみ
                    ...
                    self.stalk_count += 1  # 検出した茎の数をカウント
                    rospy.loginfo(f"Stalk position with depth published. Count: {self.stalk_count}")
                    ...
                    # すべての茎を送信したら move_flag を 1 にしてパブリッシュし、カウントをリセット
                    if self.stalk_count >= self.target_stalk_count:
                        rospy.loginfo("All stalks detected, publishing move_flag.")

                        # move_flag を 1 にしてパブリッシュ
                        self.move_flag.data = 1
                        self.move_flag_pub.publish(self.move_flag)
                        ...
                        # 10秒待機して再度認識を開始
                        rospy.loginfo("Waiting for 10 seconds before restarting recognition...")
                        rospy.sleep(10)  # 10秒待機

                        # move_flag を 0 に戻す
                        self.move_flag.data = 0
                        self.move_flag_pub.publish(self.move_flag)

                        # 再度認識を開始するために run_detection を True に設定
                        self.run_detection = True
    ...
    def task_state_callback(self, task_state_msg):
        """Callback to handle task_state messages and control stalk detection"""
        if not self.first_stalk_sent:
            rospy.loginfo("Sending first stalk position")
            self.run_detection = True  # 最初の茎位置を送信するためにフラグをセット
            self.first_stalk_sent = True
        else:
            if task_state_msg.data == self.stalk_count:
                rospy.loginfo(f"All stalks sent ({self.stalk_count} total), publishing move_flag.")
                self.move_flag.data = 1
                self.move_flag_pub.publish(self.move_flag)

                # 茎カウントをリセットし、10秒後に再度認識を開始
                self.stalk_count = 0
                rospy.sleep(10)  # 10秒待機
                self.move_flag.data = 0
                self.move_flag_pub.publish(self.move_flag)
                rospy.loginfo("Resuming stalk detection.")

                # 認識を再度開始する
                self.run_detection = True
            else:
                rospy.loginfo(f"Receiving task_state, sending next stalk position. Count: {self.stalk_count}")
                self.run_detection = True  # 次の茎を検出
