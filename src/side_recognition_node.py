...
try:
    ...
    RS_AVAILABLE = False
    print("[SideRecognition] pyrealsense2 not found. Running without RealSense intrinsics.")
...
class SideRecognitionNode:
    def __init__(self):
        rospy.init_node('side_recognition', anonymous=True)
        ...
        self.act_mask_pub = rospy.Publisher(
            "/act/side_mask/image_raw",
            Image,
            queue_size=1
        )

        # ---- RealSense キャリブレーション（横カメラ用） ----
        ...
        if RS_AVAILABLE:
            ...
            intr = rs.intrinsics()
            intr.width = camera_info.width
            intr.height = camera_info.height
            intr.ppx = camera_info.K[2]
            intr.ppy = camera_info.K[5]
            intr.fx = camera_info.K[0]
            intr.fy = camera_info.K[4]
            intr.model = rs.distortion.none
            intr.coeffs = list(camera_info.D)

            self.depth_intrin = intr
            rospy.loginfo("Side camera intrinsics set from CameraInfo")
        else:
            rospy.logwarn("SideRecognition: pyrealsense2 not available. Skip intrinsics setup.")
        ...
        self.seg_pub = rospy.Publisher(
            '/camera2/segmented_image',
            Image,
            queue_size=10
        )

        rospy.loginfo("side.recognition node started")
        rospy.loginfo(
            f"SideRecognition ACT mask enable: {self.act_mask_enable}, "
    ...
    def depth_callback(self, msg):
        try:
            ...
            rospy.logerr(f"SideRecognition: depth CvBridge error: {e}")
            return
    ...
    def load_model(self):
        ...
        if os.path.isfile(model_path):
            ...
            model.eval()
            rospy.loginfo("SideRecognition: Model loaded successfully")
        else:
            rospy.logerr(f"SideRecognition: Model checkpoint not found at {model_path}")
    ...
    def command_callback(self, msg):
        ...
        if start_flag == 1.0:
            self.run_once = True
            rospy.loginfo(f"SideRecognition: start for class {self.target_class}")
    ...
    def image_callback(self, ros_image):
        ...
        try:
            ...
            rospy.logerr(f"SideRecognition: CvBridge error: {e}")
            return
        ...
        try:
            ...
            if world_positions:
                ...
                rospy.loginfo(f"SideRecognition: Published {len(world_positions)} weeds.")

            elif pixel_positions:
                px, py = pixel_positions[0]
                result.data = [float(px), float(py)]
                self.result_pub.publish(result)
                rospy.logwarn("SideRecognition: depth not available, published pixel coords only")

            else:
                rospy.loginfo("SideRecognition: no object detected")

        except Exception as e:
            rospy.logerr(f"SideRecognition: error in image_callback: {e}")
    ...
    def detect_world_coordinates(self, cv_image, pred):
        ...
        if (not RS_AVAILABLE) or (self.depth_intrin is None) or (self.depth_image is None):
            rospy.logwarn("SideRecognition: RS or intrinsics/depth not available, skip 3D.")
        ...
        opened_mask = cv2.dilate(
            cv2.erode(original_mask, kernel, iterations=1),
            kernel,
            iterations=2
        )
        ...
        for contour in contours:
            ...
            if valid.size == 0:
                rospy.logwarn(f"SideRecognition: no valid depth around ({cX},{cY})")
                continue

            depth_value = float(np.median(valid))

            Z_m = depth_value / 1000.0 if self.depth_encoding == "16UC1" else depth_value
            ...
            try:
                ...
                rospy.logerr(f"SideRecognition: deproject error: {e}")
                continue
            ...
            rospy.loginfo(
                f"SideRecognition: pixel({cX},{cY}) -> camera"
