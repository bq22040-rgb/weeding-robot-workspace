...
class CropRecognitionNode:
    def __init__(self):
        ...
        self.csv_file_path = os.path.join(package_path, 'src', 'stalk_positions.csv')
    ...
    def command_task_callback(self, command_task):
        ...
        self.stalk_count = 0  # 新しいコマンドを受信したときに茎カウントをリセット
        self.stalk_positions = []  # 前の茎の位置リストをクリア
    ...
    def image_callback(self, ros_image):
        ...
        if self.run_detection:  # Run detection only if command_task was received
            ...
            try:
                ...
                # rospy.loginfo(f"cv_image shape: {cv_image.shape}, pred shape: {pred.shape}")
                ...
                rospy.loginfo(f"Stalk positions: {self.stalk_positions}")
    ...
    def visualize_segmentation(self, pred,cv_image):
        ...
        # predはモデルの出力クラスマップです
        decoded_segmentation = self.decode_target(pred,cv_image)  # decode_targetでカラー化
    ...
    def detect_stalk_position(self, cv_image, pred):
        ...
        closed_mask = cv2.erode(original_mask,kernel,iterations = 2)
    ...
    def publish_stalk_position(self):
        ...
        while self.stalk_positions and self.stalk_count < len(self.stalk_positions):
            ...
            #rospy.loginfo(f"Max depth in 100x100 region around stalk: {max_depth} milimeters")

            # 茎の位置を深度情報付きでpublish

            self.save_stalk_position_to_csv((cX-160)/1.5, cY/1.5, max_depth)#csv用
            ...
            rospy.loginfo(f"stalk_count:{self.stalk_count+1}/{len(self.stalk_positions)}")
            # self.run_detection = False
            
            # カウントを増やす
            self.stalk_count += 1
            rospy.loginfo("Weeding now!")
            time.sleep(10) # 10 seconds 
            self.run_detection = False
            # # 新しいcommand_taskが受信された場合は現在の処理を中断する
            # if self.run_detection:
            #     rospy.loginfo("新しいコマンドが受信されたため、現在の処理を中断します")
            #     break  # 処理を中断して新しいターゲットクラスでの認識を開始
        self.run_detection = False
        rospy.loginfo("All stalks weeded!")    

    def save_stalk_position_to_csv(self, x, y, z):
        """Save or overwrite the stalk position to a CSV file"""
        # ファイルが存在するかどうか確認する
        file_exists = os.path.exists(self.csv_file_path)
        
        # ファイルが存在する場合、上書きモードで開く (新しい行を追加するのではなく書き換え)
        with open(self.csv_file_path, mode='w', newline='') as file:
            writer = csv.writer(file)
            # 行を書き込む（ヘッダーは書き込まない）
            writer.writerow([x, y, z])
        
        rospy.loginfo(f"Saved/overwritten stalk position to CSV: ({x}, {y}, {z})")
        
        # ファイルのパーミッションを設定し、誰でも編集できるようにする
        os.chmod(self.csv_file_path, stat.S_IRWXU | stat.S_IRWXG | stat.S_IRWXO)

    def clear_csv_file(self):
        """Clear the CSV file by opening it in write mode without adding any data"""
        with open(self.csv_file_path, mode='w', newline='') as file:
            pass  # ファイルを空にするため、何も書き込まない

        # ファイルのパーミッションを設定し、誰でも編集できるようにする
        os.chmod(self.csv_file_path, stat.S_IRWXU | stat.S_IRWXG | stat.S_IRWXO)
        rospy.loginfo("Cleared CSV file")
