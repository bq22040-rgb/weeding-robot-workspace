...
class CropRecognitionNode:
    ...
    def task_state_callback(self, task_state_msg):
        ...
        if(task_state_msg.data==1){
            self.task_state_count=self.task_state_count+1
        }
        if not self.first_stalk_sent:
            rospy.loginfo("最初の茎の位置を送信中")
            self.publish_stalk_position()
            self.first_stalk_sent = True
        else:
            if task_state_msg.data == self.stalk_count:
                rospy.loginfo(f"全ての茎位置を送信しました。合計{self.stalk_count}回の送信が完了しました。")
                rospy.signal_shutdown("茎検出タスク完了")
            else:
                rospy.loginfo("次の茎位置を送信中")
                self.publish_stalk_position()

    def publish_stalk_position(self):
        """検出した茎位置を送信"""
        if self.stalk_position:
            stalk_position = Point()
            stalk_position.x = self.stalk_position[0]
            stalk_position.y = self.stalk_position[1]
            stalk_position.z = 0
            self.stalk_position_pub.publish(stalk_position)

            # 茎の数をカウント
            self.stalk_count += 1
            rospy.loginfo(f"茎位置を送信しました。カウント: {self.stalk_count}")
